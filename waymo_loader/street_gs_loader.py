...
from waymo_loader.convert import convert
from waymo_loader import colmap_text_loader

class SceneStreetWaymo(Scene):
    def __init__(self, gaussians: GaussianModel, args, waymo_raw_pkg, shuffle=True, resize_ratio=1, test=False):
        convert(args.source_path + "_colmap")
        ...
        # load colmap
        convert(args.source_path + "_colmap")
        xyzs, rgbs, w_2c = colmap_text_loader.load_data(args.source_path + "_colmap")
        w_2w = c2w_for_colmap @ w_2c.numpy()
        rgbs = rgbs / 255.0 
        homo_xyzs = np.concatenate([xyzs, np.ones((xyzs.shape[0], 1))], axis=1)
        homo_xyzs = homo_xyzs @ w_2w.T
        xyzs = homo_xyzs[:, :3] / homo_xyzs[:, 3, None]
        xyzs *= 1 # (np.max(points, axis=0) - np.min(points, axis=0))/(np.max(xyzs, axis=0) - np.min(xyzs, axis=0))

        # different init pcd
        mode = 0
        if mode == 0:
            points = np.concatenate([points, xyzs])
            colors = np.concatenate([colors, rgbs])
        elif mode == 1:
            points = xyzs
            colors = rgbs
...
def load_waymo_gs(waymo_raw_pkg, args, test):
    ...
    scene = SceneStreetWaymo(gaussians, args, waymo_raw_pkg, test=test)
